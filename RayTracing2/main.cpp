...
{
	//Por default, a cor será a resultante da iluminação ambiente
	Color color = ambientColor( object ); 
	
	//Calcular a contribuição de todas as luzes da cena para essa cor
	for (int i = 0; i < scene->getNumLights(); i++)
	{
		...
		//Vetor do ponto de interseção até a luz
		Ray L( interceptionPoint, light->position - interceptionPoint ); 

		//Verifica se o vetor L atinge algum objeto antes de chegar à luz
		//Se sim, ele não está sendo o ponto não está sendo iluminado por esta luz,
		//e será ump ponto de sombra
		...
		for ( int j = 0; j < scene->getNumObjects(); j++ )
		{
			...
			if ( obj != object )
			{
				//t: interseção do raio L e o objeto obj
				float t;
				if ( obj->intercepts( L, &t ) ) //Há um objeto ocluindo a luz
				{
					//Somente interessa a interseção que ocorre entre ponto e a luz
		...
		//Se não houve oclusão, não há sombra, então a cor é calculada:
		if ( !occlusion )
		{
			//Calcula a cor resultante da reflexão difusa
			Color dColor = diffuseColor( light, object, ray, interceptionPoint );
			
			//Calcula a cor resultante da reflexão especulada (somente se houve reflexão difusa)
			if ( dColor.r != -1.0f )
	...
	{
		//Novo raio a ser lançado a partir do ponto de interseção
...
//Função de desenho do colorChecker utilizando o OpenGL
void drawScene()
{
...
//Configuração da visualização do OpenGL
void Initialize() {
